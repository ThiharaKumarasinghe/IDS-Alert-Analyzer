...
background_image = Image.open("FileImport/background_fileImport.png")  # Ensure this path is correct
...
loading_label = ctk.CTkLabel(right_frame, text="Processing... Please wait.", font=("Montserrat", 14))
loading_label.grid(row=6, column=0, sticky="nsew", padx=10, pady=10)
loading_label.grid_remove()  # Hide the loading label initially

# Function to handle the upload and processing in a separate thread
def handle_upload():
    loading_label.grid()  # Show the loading label
    upload_pcap_file()  # Process the file upload and conversion
    loading_label.grid_remove()  # Hide the loading label after processing

# Upload Button
upload_button = ctk.CTkButton(right_frame, text="Upload PCAP File", command=lambda: threading.Thread(target=handle_upload).start())
